...
import { CPH_MODEL_DEFINITIONS } from "@/models/cph-model/definitions";
import type {
  CapacityDataRow,
  TeamPeriodicMetrics,
...
export default function CPHModelPage() {
  const [selectedModel] = useState<ModelType>('cph');
    ...
    Object.entries(buGroups).forEach(([buName, buEntries]) => {
      ...
      displayedPeriodHeaders.forEach(period => {
        ...
        buEntries.forEach(lobEntry => {
          ...
          lobEntry.teams.forEach(teamEntry => {
            ...
            const calculatedTeamMetrics = calculateCPHTeamMetricsForPeriod(
              teamInputData,
              lobTotalBaseMinutes,
      ...
      buEntries.forEach(lobEntry => {
        ...
        displayedPeriodHeaders.forEach(period => {
          ...
          // Convert AHT to CPH for display
          const lobAverageCPH = lobAHT > 0 ? 60 / lobAHT : null;

          lobPeriodicData[period] = {
            lobVolumeForecast: lobVolume,
            lobAverageAHT: lobAHT,
            averageCPH: lobAverageCPH, // CPH-specific field
    ...
    setRawCapacityData(prevData => {
      return prevData.map(lobEntry => {
        if (lobEntry.id === lobId) {
          ...
          // Handle CPH to AHT conversion
          if (metricKey === 'lobAverageAHT' && selectedModel === 'cph') {
            // If user is editing CPH, convert to AHT for storage
            const cphValue = numericValue;
            const ahtValue = cphValue && cphValue > 0 ? 60 / cphValue : null;
    ...
    <div className="flex flex-col h-full bg-background text-foreground">
      ...
      <main className="flex-grow overflow-hidden">
        <CapacityTable
          ...
          teamMetricDefinitions={CPH_MODEL_DEFINITIONS.teamMetrics}
          aggregatedMetricDefinitions={CPH_MODEL_DEFINITIONS.aggregatedMetrics}
